...
def process_paper_with_correct_scores(row):
    ...
    reviewers_comments = []
    for i in range(1, 5):  # Assuming up to 3 reviewers
    ...
    scores = {
        "Relevance": row.get("CryptoEx Review: Relevance", ""),
        "Content and originality": row.get("CryptoEx Review: Technical content and originality", ""),
        "Reference": row.get("CryptoEx Review: Reference to Related Work", ""),
        "Overall recommendation": row.get("CryptoEx Review: Overall Recommendation about accepting the contribution", ""),
        "Poster acceptance": row.get("CryptoEx Review: Poster acceptance", "")
    }
...
# Example usage:
input_excel_file = 'ieeecryptoex24-papers.xlsx'  # Replace with your input Excel file path
output_json_file = 'ieeecryptoex24-papers.json'  # Replace with your desired output JSON file path
